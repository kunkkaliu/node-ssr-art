...
const startRouter = (router, koaCache) => {
    router.get('/', /* koaCache(10 * 60), */ async (ctx, next) => {
        ...
        if (useTime > 500) {
            ctx.logger.warn('api', `access /list/getList use ${useTime}ms`);
        }
        ctx.logger.info('api', `access /list/getList use ${useTime}ms`);
        // 根据不同结果渲染不同页面
        if (res && res.data && res.data.code === 0) {
            ...
            ctx.logger.warn('api-result', `/list/getList ${res && res.data && res.data.message}`);
